...
function createStairs() {
  ...
  for (var index = 0; index < 6; ++index) 
  {
    ...
    stepMesh.position.y = verticalStepHeight/2 + index*(verticalStepHeight + stepThickness);	// half of height: put it above ground plane
    stepMesh.position.z = index*(horizontalStepDepth - stepThickness);			// centered at origin
    scene.add( stepMesh );
    ...
    stepMesh.position.y = stepThickness/2 + verticalStepHeight + index*(verticalStepHeight + stepThickness);
    // Push step forward by half the depth, minus half the vertical step's thickness
    stepMesh.position.z = horizontalStepDepth/2 - stepHalfThickness + index*(horizontalStepDepth - stepThickness);
    scene.add( stepMesh );
